...
# Need to fix code to accomodate for lower precision
class Linear:
...
q = Tensor.randn(b,h,s,d_k)
k = Tensor.randn(b,h,s,d_k)
v = Tensor.randn(b,h,s,d_k)
mask = Tensor.ones(s,s).triu(diagonal=1).unsqueeze(0).unsqueeze(0)
#print(mask.numpy())
output, attn_weights = scaled_dot_product_attn(q,k,v,mask)
